 ...
 */
class AppClickImageView @JvmOverloads constructor(
    ...
    private var activeImgNight : Drawable? = null
    ...
    var onClickViewState: OnClickViewState?=null
    ...
    private fun setImgDrawable(img:Drawable?){
        img?.let { setImageDrawable(it) }
    }
    ...
    private fun setActiveUI(b:Boolean){
        if(b){
            ...
            setImgDrawable(activeImg)
        }else{
            setBackgroundColor(mInitialBackgroundColor)
            setCustomDrawable(mInitialBackground)
            setImgDrawable(mInitialImg)
        }
    }
    ...
    private fun setActiveNightUI(b:Boolean){
        if(b){
            ...
            setImgDrawable(activeImgNight)
        }else{
            setBackgroundColor(bgColorNight)
            setCustomDrawable(drawableNight)
            setImgDrawable(imgNight)
        }
    }
    ...
    fun setSwitch(b:Boolean){
        mActive = b
        // 更新 UI 来显示新的图片
        notifyUI(AppMK.getAppTheme())
        onClickViewState?.onClick(b)
    }
    ...
    private fun initView(theme: Int){
        ...
        typedArray?.apply {
            ...
            activeImgNight = getDrawable(R.styleable.BaseStyles_activeImgNight)
            ...
            drawableNight = getDrawable(R.styleable.BaseStyles_activeDrawable)
            imgNight = getDrawable(R.styleable.BaseStyles_imgNight)
