...
object LiveDataGate {
    ...
    fun <T> output(content: LifecycleOwner, key: String, type: Class<T>, callback: (T?) -> Unit) {
        when(content){
    ...
    fun <T> getEntity(key: String, type: Class<T>): T? {
        ...
        // 如果是基本类型(String, Int等)，Gson解析可能不如直接强转，
        // 但为了统一逻辑，通常 SP 存的是 String。
        // 为了兼容旧的 put(String, Int) 逻辑，我们需要做个特判：

        // 修正逻辑：如果内存没命中，说明是基本类型存在 SP 里，或者是 Gson 串
        // 简单策略：尝试从 SPUtils 获取原生类型
